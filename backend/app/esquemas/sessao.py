...
(frontend/src/services/sessionsService.js): campos ``id``, ``name``,
``sourceFile`` (camelCase), ``date``, ``duration``, ``frames``,
``predominant``, ``confidence``, ``probabilities`` e ``timeline``, com
chaves de emoção em português.

Os nomes de campo ficam em inglês de propósito: são o contrato de API já
consumido pelo frontend e não mudam com a padronização em português do
restante do backend (ver app.dominio.sessao para a entidade em português).
"""
